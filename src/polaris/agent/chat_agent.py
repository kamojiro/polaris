...
walking skeleton の範囲では確認を挟まず、URL が来たら即座に保存する
(受け入れ条件「URLを貼ってから一覧に反映されるまで一往復で完結する」を満たすため)。
"""
...
def build_chat_agent(settings: Settings, repo: PaperRepository) -> Agent:
    """設定とリポジトリから論文チャットエージェントを組み立てる."""
    ...
    async def save_paper(url: str) -> str:
        ...
        """
        try:
            item, record, created = await ingest_paper_from_url(url, repo=repo, http_client=http_client)
        except InvalidPaperUrlError:
            return f"'{url}' から arXiv の論文IDを特定できませんでした。"

        authors = "、".join(record.authors) if record.authors else "不明"
        status = "新規に保存しました" if created else "既に保存済みでした"
        return f"{status}: 『{item.title}』(著者: {authors}, arXiv:{record.arxiv_id})"
