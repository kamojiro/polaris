...
## 将来構想: 自律調査ループ(2026-09-13)

いずれ、テーマ/仮説を自分で立てる→調査(027の引用チェイニング等)→検証・実験(コードを書いて実際に実行する想定)→まとめる→次に何を調べるか自分で決める、というループが裏で常時回り続け、ユーザーは023のバナー/008のダイジェストのような形で定期的に結果を覗くだけ、という状態にしたい。027のストーリー2(継続調査)・ストーリー4(再帰的サブテーマ分解)は、単体の拡張というより、この自律ループへ向けた布石として位置づけ直せる。

コードを書いて実際に実行する「実験」まで含める構想(文献の突き合わせだけでなく、仮説をコードで検証するところまで自律化したい)。ただしこれは「いずれやりたい」レベルで、着手には準備が要ると認識している(即着手ではない)。

`alphaXiv/OpenResearch`(2026-09-13に共有)がこの「実験」部分の参考になりうる。Claude Code/Codex/OpenCodeをgit worktreeで並列に動かし、提案→コード変更→実験実行→検証→次の一手、というautoresearchループを回すローカルファーストのワークスペース(Rust製CLI、`orx discover keyword`/`orx paper`という文献発見系サブコマンドも持つが、本体は実験オーケストレーション)。Polaris本体(文献の調査・要約)とは責務を分けてもよく、Polarisは「定期的に確認する」窓口(通知・ダッシュボード)側を担い、自律ループの実行基盤自体は将来的に別ツール(OpenResearchやその類)に任せる、という分担もありうる。現時点では方向性のメモに留め、具体的な統合方式(Polarisから叩くのか、完全に別プロセスとして走らせて結果だけ拾うのか等)は未検討。

**OpenResearchのSYSTEM_PROMPT.md/SKILL.mdから拾えた設計原則(2026-09-13)**:

- 「一度結果が出たノードは編集せず不変とし、試したければ子ノードを分岐する」という原則。017/019の「ログ層(追記のみ)+現在状態層」パターンと同じ発想で、方向性の裏付けになる
- 「1ラウンドで少し横に試したら、一番良かったものへ深掘りして降りていく。ルートに子が並ぶだけでその先(孫)が無いのが失敗パターン」という木の伸ばし方の原則。自律調査ループが際限なく広く浅く探索し続けるのを防ぐ具体的な指針になりうる
- ラウンド完了後の意思決定を「repair(やり直す)/refill(別の枝を試す)/promote(深掘りして降りる)/stop(終える)」の4択に絞るという型。将来ループの状態遷移設計にそのまま使えそう
- 発見ソースとして`alphaXiv`自体の全文検索・意味検索に加えOpenAlex(学際的な学術グラフ)・bioRxivも使っている。027は現状Semantic Scholarのみなので、将来ソースを増やす際の候補として記録

なお「根拠明示(実質的な主張には直後に出典を添える、推論は観測と区別する)」という規律は、027の統合ステップに直接取り込める改善として`specs/027-related-paper-research/spec.draft.md`に反映済み。
...
統合段(refine方式)の「セクションが少なすぎる」問題と対応方針(アウトライン先行方式への作り直し、2026-09-13実装完了)は`specs/027-related-paper-research/spec.draft.md`「改善: 統合結果が薄い問題」を参照(このIDEAS.mdに書いていた旧メモは、specの詳細な設計・実装完了により役目を終えたため削除した)。
