...
- Relate: 記事間のトピック類似度(`Chunk`/`Embedding`基盤を使う)による`Relation`エッジ、および情報源ラベルによるグルーピング。**2026-08-26追記**: `docs/adr/0011-pause-paper-embedding-generation.md`で論文Ingest時のembedding生成は一時停止中(消費する機能が無かったため)。Phase Bはembeddingの具体的な消費者になるので、着手時に生成を再開する
...
### 表示の改善: ニュース一覧のカード化・ピックアップサイドバー(2026-08-27〜28)

「内容はいいが見づらい」というフィードバックを受けて、Phase Aのスコープには無かった表示改善を2段階で行った。

- `NewsList.tsx`(チャットの`list_news`結果): source_labelごとに左のアクセント色を付け、上位5件のみ常時表示・残りは`<details>`で折りたたむようにした(1ラベル最大15件を一度にベタ書きすると読みづらかったため)
- ニュースピックアップサイドバー(`Sidebar.tsx`+`NewsSidebar.tsx`): チャット欄右側に、取り込み済みニュースからランダムに5件選んで常時表示する新しい導線。`list_news`(チャットの発言に応じてのみ表示される)とは独立に、ページを開いた時点でアンビエントに見せたいため、専用のREST endpoint(`GET /api/news/picks`)を新設した。表示用の短い見出しは選ばれた数件分だけ`agent/sidebar_title.py`(`structure_news.py`と同型の狭いタスクエージェント)で生成する(arXivの日次流入全件を判定するような設計は避けている)。クリックすると`『{タイトル}』について詳しく教えて {URL}`というメッセージを送り、通常の会話の流れで詳しい説明を引き出す(`PaperList.tsx`の「タイトルクリックで論文モードへ」と同じ考え方)。arXivのURLは既存のsave_paper自動判定に乗って全文ベースの論文モードに入り、それ以外はエージェントが持っている情報(+必要ならweb_search)で答える(専用の全文取得アダプタは無い)
