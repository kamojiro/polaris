...
1. **011のADR-0013対応(chat_agent.pyのドメイン別分割)** — 依存なし、採択済みで着手可能。実行時の構成は変えないファイル整理のみ(011本体の固定チーム型マルチエージェント化とは別、そちらは引き続き保留)
...
10. 011 (agent-registry) — 固定チーム型マルチエージェント化本体。ADR-0013のファイル分割とは別、引き続き着手トリガー待ち
