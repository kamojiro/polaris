...
`#`(採番順)とは別に、実際に着手する順番はこちら。フェーズ内は上から順に、依存関係も考慮済み。003・007・014・015・017・018は完了済みのため対象外(003/015の追加提案3件も2026-08-26に実装完了)。008もPhase Aが完了済み(Phase Bのみ009待ちで残る)。ただし015には2026-08-29追記のADR-0012対応(下記1)が未実装で残っている。

1. **015のADR-0012対応(会話履歴トリミング)** — 依存なし、設計済みで着手可能。`get_paper_full_text`の全文が履歴に蓄積し続ける問題への対処。013がIR文書の全文取得toolを持つ前に片付けておきたい
2. **013 (ir-analysis-domain)** — 依存なし、spec詳細化済みで着手可能
3. **023 (daily-summary-notification)** — 依存なし(002/007/013/017の既存ドメインだけで動く設計)、spec詳細化済みで着手可能。013と並行でも順不同でもよい
4. 024 (memory-theme-housekeeping) — 詳細化未着手・優先度も未定。023のパターンを再利用する着想メモ
5. 025 (ir-tracking-expansion) — 詳細化未着手・優先度も未定。013への追加。Stage 1(既存追跡企業の新規開示チェック)は詳細化済み
6. ~~004 (citation-relations)~~ — 見送り
7. ~~005 (eval-harness)~~ — いつかやるリストへ(下記参照)
8. 006 (chatlog-backfill) — 005に依存するため005が動くまで自動的に後回し
9. 016 (paper-structured-parsing) — 015を使ってみて図表QA・引用根拠が必要になったら着手(スケルトンのみ、着手トリガー待ち)
10. 009 (dashboard) — 008 Phase B・010の依存元
11. 010 (mobile-pwa) — 009に依存
12. 011 (agent-registry)
13. 012 (local-llm-cutover)
14. 019 (diary-domain) — 詳細化未着手・優先度も未定。着想メモのみ
15. 020 (google-workspace-integration) — 詳細化未着手・優先度も未定。着想メモのみ
16. 021 (discord-integration) — 詳細化未着手・優先度も未定。着想メモのみ
17. 022 (misskey-integration) — 詳細化未着手・優先度も未定。着想メモのみ(投稿は許可制の方針のみ決定済み)
18. 026 (voice-input) — 詳細化未着手・優先度も未定。Stage 1(プッシュトゥトーク+STT、003に合流)のみ着想済み、Stage 2(常時リスニング+発話分類)は将来
