...
`023-daily-summary-notification`と同じバナーで通知する。クリック後の展開先は、`019-diary-domain`で2026-09-15に決定した「サイドバーを一時的に隠し、画面を左右半分に分割してチャットと統合結果を同格で並べる」canvas的な表示パターンを採用する(019の「気になっていること」参照)。中央ポップアップ・専用カラム追加等の他候補は019側の検討で不採用済みのため、027で改めて検討し直す必要はない。
