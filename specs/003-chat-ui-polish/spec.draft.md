...
## 気になっていること: composerのアイコンが多い(2026-09-14、026実装により顕在化)
...
## 気になっていること: ウェイクワードモード時の二重送信(2026-09-14、対応中)

`026-voice-input`のウェイクワードモード使用中に、返信が返ってくる前に2回連続で送信してしまうことがある(2文発話→2回検知→2回`sendMessage()`、または1発話が2回検知される、のどちらかは要調査)。**2026-09-14時点で対応中。**

- 通常のプッシュトゥトーク(ボタン押下)では起きていない問題なので、ウェイクワード検知の連続トリガー制御(「送信中は次の検知を無視する」ようなガード)が抜けている可能性が高い
- `isRunning`のような既存の「送信中フラグ」をウェイクワード検知トリガー側でもチェックする形にできないか、026の実装時に確認したい
