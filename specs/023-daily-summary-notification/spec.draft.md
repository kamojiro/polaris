...
✔️ 完了(v1、2026-08-30)。013/019が実装されたら集計対象に追加可能な構造にしてある(013は2026-08-30に実装完了、集計への追加はまだ未着手)
...
✔️完了(v1)。実装当時`013-ir-analysis-domain`は未実装だったため集計対象から除外した。
**2026-08-30追記**: 013実装完了。`collect_activity`へのIR文書集計ブロック追加はまだ未着手
(この関数を書いた時点で見込んだとおり、`IrRepository`に`list_ir_documents_created_between`
相当のメソッドを1つ足して`DailyActivity`に1フィールド足すだけで済むはず)。
